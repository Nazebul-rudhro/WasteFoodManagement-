...
class DonorProvider extends ChangeNotifier {
  ...
  void fetchAllRequests() {
    ...
    // ১. রিসিভারের পাঠানো পেন্ডিং রিকোয়েস্ট (যা ডোনার এখনো এক্সেপ্ট করেনি)
    _db.collection('requests')
        .where('donorId', isEqualTo: user.uid)
        .where('status', isEqualTo: 'pending')
        .snapshots().listen((snapshot) {
    ...
    // ২. অ্যাপ্রুভড এবং ডেলিভারি লিস্ট (যা প্রসেসিংয়ে আছে বা শেষ হয়েছে)
    _db.collection('requests')
        .where('donorId', isEqualTo: user.uid)
        .where('status', whereIn: ['approved', 'delivered', 'ongoing', 'completed'])
        .snapshots().listen((snapshot) {
      approvedRequests = snapshot.docs.map((doc) => {"requestId": doc.id, ...doc.data()}).toList();
      notifyListeners();
    });
  ...
  // ✅ ডোনার যখন ভলান্টিয়ারকে হ্যান্ডওভার করবে
  Future<void> assignVolunteer(String requestId, String volunteerId, String volunteerName) async {
    try {
      await _db.collection('requests').doc(requestId).update({
        'volunteerId': volunteerId,
        'volunteerName': volunteerName,
        'status': 'delivered',        // ডোনারের সাইড থেকে কাজ শেষ
        'deliverystatus': 'pending',  // ভলান্টিয়ারের সাইড থেকে এখন 'To Pickup' মোডে আছে
        'assignedAt': FieldValue.serverTimestamp(),
      });
    } catch (e) {
      debugPrint("Assign Volunteer Error: $e");
  ...
  // ✅ ডোনার যখন রিসিভারের রিকোয়েস্ট Approve বা Reject করবে
  Future<void> handleRequest(String requestId, String postId, String action) async {
    try {
      WriteBatch batch = _db.batch();
      DocumentReference reqRef = _db.collection('requests').doc(requestId);
      ...
      if (action == 'approved') {
        batch.update(reqRef, {
          'status': 'approved',
          'deliverystatus': 'none', // এখনো কোনো ভলান্টিয়ার নেই
          'approvedAt': FieldValue.serverTimestamp()
        });
        batch.update(postRef, {'status': 'claimed'});

        // ঐ একই পোস্টের অন্য সব পেন্ডিং রিকোয়েস্টগুলো অটোমেটিক রিজেক্ট করে দেওয়া
        final others = await _db.collection('requests')
            .where('postId', isEqualTo: postId)
            .where('status', isEqualTo: 'pending').get();
